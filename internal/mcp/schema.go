...
func AllSchemas() []Schema {
	return []Schema{
		{Name: "connect", Description: "Pair with a remote share by providing the assist code. Must be called first before any other tool. The other tools will return 'not_connected' until this succeeds. Optionally override the relay server address (server='host:port') when the user says the share is on a specific LAN address — this is how 'share --standalone' is used.", InputSchema: json.RawMessage(`{"type":"object","required":["code"],"properties":{"code":{"type":"string","description":"Assist code from the share side, e.g. 'ABCD-EFGHIJ' or 'ABCDEFGHIJ' (hyphen optional)"},"server":{"type":"string","description":"Optional relay address override (host:port). Use when the user says the share is at a specific LAN IP, e.g. '192.168.137.13:8443'. Skips the default relay in .mcp.json."}}}`)},
