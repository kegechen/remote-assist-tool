...
func TestDeriveSessionKeyDeterministic(t *testing.T) {
	k1 := DeriveSessionKey("CODE-1234", "NONCE_S", "NONCE_H", ToolProtocolVersion)
	k2 := DeriveSessionKey("CODE-1234", "NONCE_S", "NONCE_H", ToolProtocolVersion)
...
func TestDeriveSessionKeyDiffersByNonce(t *testing.T) {
	k1 := DeriveSessionKey("CODE-1234", "A", "B", ToolProtocolVersion)
	k2 := DeriveSessionKey("CODE-1234", "A", "C", ToolProtocolVersion)
	if bytes.Equal(k1[:], k2[:]) {
