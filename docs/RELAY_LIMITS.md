...
- 工具通道（`tool_req` / `tool_resp` / `tool_stream`）的 AEAD 带 AAD，绑定 `tool` / `id` / `deadline_ms` / `seq` / `stream` 这些外层明文字段；握手后 args 必须是合法密文；接收侧按调用 ID 做 1024 位滑动窗口抗重放。三者合起来构成工具通道协议 v2，与 v1 不互通，版本号在握手阶段就会把旧端挡掉。
